...
func main() {
	...
	err = inst.Transition(reviewState.ID, "submit", systemActor, metadata1, nil)
	if err != nil {
	...
	err = inst.TransitionWithSubState(reviewState.ID, subStateQA, "submit", systemActor, metadata2, nil)
	if err != nil {
		printError(fmt.Sprintf("Transition failed: %v", err))
	} else {
		tracker.Track(inst.DomainEvents())
	...
	err = inst.TransitionWithSubState(reviewState.ID, subStateCompliance, "submit", systemActor, metadata3, nil)
	if err != nil {
		printError(fmt.Sprintf("Transition failed: %v", err))
	} else {
		tracker.Track(inst.DomainEvents())
	...
	err = inst.Transition(revisionsState.ID, "request_revisions", systemActor, metadataWithFeedback, nil)
	if err != nil {
		printError(fmt.Sprintf("Transition failed: %v", err))
	} else {
		tracker.Track(inst.DomainEvents())
	...
	err = inst.Transition(reviewState.ID, "resubmit", systemActor, metadata4, nil)
	if err != nil {
		printError(fmt.Sprintf("Transition failed: %v", err))
	} else {
		tracker.Track(inst.DomainEvents())
	...
	err = inst.Transition(approvedState.ID, "approve", systemActor, metadata5, nil)
	if err != nil {
		printError(fmt.Sprintf("Transition failed: %v", err))
	} else {
		tracker.Track(inst.DomainEvents())
	...
	err = inst.Transition(rejectedState.ID, "reject", systemActor, instance.EmptyTransitionMetadata(), nil)
	if err != nil {
	...
	inst2.Transition(reviewState.ID, "submit", systemActor, instance.NewTransitionMetadataWithReason("Submitting policy"), nil)
	tracker.Track(inst2.DomainEvents())
